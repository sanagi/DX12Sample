...
void Material::Load(ComPtr<ID3D12Device> device, FILE* fp, std::string modelPath) {
	...
	_pmdMaterialVector = std::vector<PMDMaterial>(_materialNum);
	...
	auto tmp = _pmdMaterialVector.size() * sizeof(PMDMaterial);

	fread(_pmdMaterialVector.data(), _pmdMaterialVector.size() * sizeof(PMDMaterial), 1, fp); //全部読む
