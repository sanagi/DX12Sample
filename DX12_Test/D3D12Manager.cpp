...
D3D12Manager::D3D12Manager(HWND hwnd) : _windowHandle(hwnd) {
	...
	//描画オブジェクト作る
//どっか別でやりたい
	_model = new Model(_device, "Model/初音ミク.pmd", "rb");

	//行列
	_matrix = new Matrix(_device, _winSize.cx, _winSize.cy, basicHeapHandle);
}
...
HRESULT D3D12Manager::Render() {
	...
	_commandList->SetDescriptorHeaps(1, _resourceHeaps.GetAddressOf());
	_commandList->SetGraphicsRootDescriptorTable(0, _resourceHeaps.Get()->GetGPUDescriptorHandleForHeapStart());

	//描画コマンド
	_model->Draw(_device, _commandList);
